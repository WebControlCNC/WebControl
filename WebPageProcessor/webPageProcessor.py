...
class WebPageProcessor:
    ...
    def createWebPage(self, pageID, isMobile, args):
        ...
        settingsTemplate = "settings_mobile.html" if isMobile else "settings.html"

        enableCustom = self.data.controllerFirmwareVersion >= 100
        fourMotor = self.data.controllerFirmwareVersion >= 100

        home = self.data.config.getHome()
        gcodeDir = f"{home}/.WebControl/gcode"
        boardsDir = f"{home}/.WebControl/boards"
        ...
        if pageID == "maslowSettings":
            ...
            gpioTemplate = "gpio_mobile.html" if isMobile else "gpio.html"
            ...
            files = []
            try:
                for _root, _dirs, _files in os.walk(gcodeDir):
                    if _dirs:
                        directories = _dirs
                    for file in _files:
                        if _root != gcodeDir:
                ...
                print(e)
           # files = [f for f in listdir(gcodeDir) if isfile(join(gcodeDir, f))]
            ...
            if self.data.pyInstallUpdateAvailable:
                ...
                updateRelease = "N/A"
            firmwareSupportsZaxisLimit = float(self.data.controllerFirmwareVersion) > 1.26
            ...
            zAxisTemplate = "zaxis_mobile.html" if isMobile else "zaxis.html"
            page = render_template(
                zAxisTemplate, 
            ...
            distToMoveZ = self.data.config.getValue("Computed Settings", "distToMoveZ")
            unitsZ = self.data.config.getValue("Computed Settings", "unitsZ")
            setZAxisTemplate = "setZaxis_mobile.html" if isMobile else "setZaxis.html"
            page = render_template(
                setZAxisTemplate, 
                minZlimit = minZlimit, 
                maxZlimit = maxZlimit, 
            ...
            socketio.emit("closeModals", {"data": {"title": "Actions"}}, namespace=self.namespace)
            setSprocketsTemplate = "setSprockets_mobile.html" if isMobile else "setSprockets.html"
            page = render_template(setSprocketsTemplate, chainExtendLength=chainExtendLength, fourMotor=fourMotor)
            return page, "Set Sprockets", False, "medium", "content", False
        elif pageID == "resetChains":
            chainExtendLength = self.data.config.getValue("Advanced Settings", "chainExtendLength")
            socketio.emit("closeModals", {"data": {"title": "Actions"}}, namespace=self.namespace)
            resetChainsTemplate = "resetChains_mobile.html" if isMobile else "resetChains.html"
            page = render_template(resetChainsTemplate, chainExtendLength=chainExtendLength, fourMotor=fourMotor)
            ...
            if self.data.units == "MM":
                scale = 25.4
                units = "mm"
            editBoardTemplate = "editBoard_mobile.html" if isMobile else "editBoard.html"
            page = render_template(
                editBoardTemplate,
                units=units,
            ...
            if self.data.units == "MM":
                scale = 25.4
                units = "mm"
            # Yep the mobile version is the same
            trimBoardTemplate = "trimBoard.html" if isMobile else "trimBoard.html"
            page = render_template(trimBoardTemplate, units=units, pageID="trimBoard")
            ...
            files = []
            try:
                for _root, _dirs, _files in os.walk(boardsDir):
                    if _dirs:
                        directories = _dirs
                    for file in _files:
                        if _root != boardsDir:
                ...
                print(e)
            # files = [f for f in listdir(boardsDir) if isfile(join(boardsDir, f))]
            directories.insert(0, "./")
            if lastSelectedDirectory is None:
                lastSelectedDirectory = "."
            page = render_template(
